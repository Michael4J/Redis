...
public class RedisDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 这个是最简单的redis连接示例，不过不推荐
//        Jedis jedis = new Jedis("192.168.43.128", 6379);    // 连接 Redis 服务
//        jedis.auth("redis");    // 设置密码
//        System.out.println("Server is running: " + jedis.ping());   // 查看服务是否运行
    ...
    private static void init() {
        ...
        jedisPool = new JedisPool(config, "192.168.43.128", 6379, 5000, "redis", 0); // 配置、ip、端口、连接超时时间、密码、数据库编号（0~15）
    }
    ...
    private static void sets() {
        ...
        System.out.println("差集放入新集合中：" + set);
        System.out.println();
...
//        pubSub.proceed(jedis.getClient(), "channel");   // 开始监听channel频道的消息
//        pubSub.unsubscribe();   // 取消监听
        jedis.subscribe(pubSub, "channel"); // 订阅channel频道的消息
